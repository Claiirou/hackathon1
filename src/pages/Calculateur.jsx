...
function Calculateur() {
  ...
  setIsChosen(value);
  usersChoice.push(toutLalcool.filter((oui) => oui.nom_francais === value)[0]);
}

  function pickOneFood (value) {
    setIsChosenFood(value);
  usersChoiceFood.push(touteLaBouffe.filter((non) => non.nom_francais === value)[0]);
}
  function addItAllUp() {
    const syntheseAlcool = [];
    usersChoice.forEach(ouestcequonva => syntheseAlcool.push(ouestcequonva.impact_environnemental['Changement climatique'].synthese / 2));
    const syntheseFood = [];
